...
class MEC_feature_schema extends MEC_base
{
    ...
    public function meta_box_schema($post)
    {
        ...
        ?>
        <div class="mec-meta-box-fields mec-event-tab-content mec-panel" id="mec-schema">
            <div class="mec-panel__head">
                <h4 class="mec-panel__title"><?php echo esc_html__('SEO Schema', 'modern-events-calendar-lite'); ?></h4>
            </div>
            <p class="mec-field__help" style="margin-bottom:18px;"><?php esc_html_e("Following statuses are for informing search engines (Google, bing, etc) about your events so they can manage your events better. Therefore you can use these statuses to be more Search Engine Friendly.", 'modern-events-calendar-lite'); ?></p>
            ...
            <div class="mec-form-row mec-field mec-field--check">
                <label class="mec-check">
                    <input class="mec-schema-event-status" type="radio" name="mec[event_status]"
                           value="EventScheduled" <?php echo($event_status == 'EventScheduled' ? 'checked="checked"' : ''); ?>>
                    <span><?php esc_html_e('Scheduled', 'modern-events-calendar-lite'); ?></span>
                </label>
                <p class="mec-field__help"><?php esc_html_e('For active events!', 'modern-events-calendar-lite'); ?></p>
            </div>
            <div class="mec-form-row mec-field mec-field--check">
                <label class="mec-check">
                    <input class="mec-schema-event-status" type="radio" name="mec[event_status]"
                           value="EventRescheduled" <?php echo($event_status == 'EventRescheduled' ? 'checked="checked"' : ''); ?>>
                    <span><?php esc_html_e('Rescheduled', 'modern-events-calendar-lite'); ?></span>
                </label>
                <p class="mec-field__help"><?php esc_html_e('For rescheduled events!', 'modern-events-calendar-lite'); ?></p>
            </div>
            <div class="mec-form-row mec-field mec-field--check">
                <label class="mec-check">
                    <input class="mec-schema-event-status" type="radio" name="mec[event_status]"
                           value="EventPostponed" <?php echo($event_status == 'EventPostponed' ? 'checked="checked"' : ''); ?>>
                    <span><?php esc_html_e('Postponed', 'modern-events-calendar-lite'); ?></span>
                </label>
                <p class="mec-field__help"><?php esc_html_e('If you postponed an event then you can use this status!', 'modern-events-calendar-lite'); ?></p>
            </div>
            <div class="mec-form-row mec-field mec-field--check">
                <label class="mec-check">
                    <input class="mec-schema-event-status" type="radio" name="mec[event_status]"
                           value="EventCancelled" <?php echo($event_status == 'EventCancelled' ? 'checked="checked"' : ''); ?>>
                    <span><?php esc_html_e('Cancelled', 'modern-events-calendar-lite'); ?></span>
                </label>
                <p class="mec-field__help"><?php esc_html_e('If you cancelled an event then you should select this status!', 'modern-events-calendar-lite'); ?></p>
            </div>
            <div id="mec_cancelled_reason_wrapper"
                ...
                <div class="mec-form-row mec-field">
                    <div class="mec-field__label">
                        <label for="mec_cancelled_reason"><?php esc_html_e('Reason for Cancellation', 'modern-events-calendar-lite'); ?></label>
                    </div>
                    <div class="mec-field__control">
                        <input type="text" id="mec_cancelled_reason" name="mec[cancelled_reason]"
                               value="<?php echo esc_attr($cancelled_reason); ?>"
                               placeholder="<?php esc_attr_e('Please write your reasons here', 'modern-events-calendar-lite'); ?>">
                        <p class="mec-field__help"><?php esc_html_e('This will be displayed in Single Event and Shortcode/Calendar Pages', 'modern-events-calendar-lite'); ?></p>
                    </div>
                </div>
                <div class="mec-form-row mec-field mec-field--check">
                    <label class="mec-check" for="mec_display_cancellation_reason_in_single_page">
                        <input
                            <?php
                        ...
                        <span><?php esc_html_e('Display in single event page', 'modern-events-calendar-lite'); ?></span>
                    </label>
                </div>
            </div>
            <div class="mec-form-row mec-field mec-field--check">
                <label class="mec-check">
                    <input class="mec-schema-event-status" type="radio" name="mec[event_status]"
                           value="EventMovedOnline" <?php echo($event_status == 'EventMovedOnline' ? 'checked="checked"' : ''); ?>>
                    <span><?php esc_html_e('Moved Online', 'modern-events-calendar-lite'); ?></span>
                </label>
                <p class="mec-field__help"><?php esc_html_e('For the events that moved online!', 'modern-events-calendar-lite'); ?></p>
            </div>
            <div id="mec_moved_online_link_wrapper"
                 class="event-status-schema" <?php echo($event_status == 'EventMovedOnline' ? '' : 'style="display: none;"'); ?>>
                <div class="mec-form-row mec-field">
                    <div class="mec-field__label">
                        <label for="mec_moved_online_link"><?php esc_html_e('Online Link', 'modern-events-calendar-lite'); ?></label>
                    </div>
                    <div class="mec-field__control">
                        <input type="url" id="mec_moved_online_link" name="mec[moved_online_link]"
                               value="<?php echo esc_url($moved_online_link); ?>"
                               placeholder="<?php esc_attr_e('https://example.com/online-event', 'modern-events-calendar-lite'); ?>">
                        <p class="mec-field__help"><?php esc_html_e('Link to join online event. If you leave it empty event link will be used.', 'modern-events-calendar-lite'); ?></p>
                    </div>
                </div>
            </div>
        </div>
